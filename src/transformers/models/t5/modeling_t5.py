...
class T5LayerNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states.to((self.weight).device)
...
class T5NodeCrossAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.NodeAttention = T5Attention(config, has_relative_attention_bias=False)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        if not self.is_decoder:
            self.node_cross_attention = T5NodeCrossAttention(config)
    ...
    def forward(
        ...
        if not self.is_decoder:
            ...
            # # tensor = torch.stack(list(graph_batch.values()))
            
            # # #tensor = torch.tensor(graph_batch)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        ```"""
        # if graph_batch==None:
        #     print("no graph batch")
        # else:
        #     print("graph batch passed")
        #     print(type(graph_batch))
